...
app_dynamic= Flask(__name__, static_url_path='/static')
CORS(app_dynamic, resources={r"/*": {"origins": "*"}})
# Load the model
...
@app_dynamic.route('/predict', methods=['GET'])
def predict():
    ...
    if prediction[0] == 0:
        reasons = {}
        if input_df['previous_year_rating'].iloc[0] < 3:
            reasons['Poor performance'] = 'Improve performance in current role'
        if input_df['avg_training_score'].iloc[0] < 70:
            reasons['Low training score'] = 'Complete additional training courses'
        if input_df['length_of_service'].iloc[0] < 3:
            reasons['Short tenure'] = 'Gain more experience in current role'
        if input_df['average_montly_hours'].iloc[0] < 200:
            reasons['Low completed monthly hours'] = 'Increase your average monthly hours to demonstrate greater commitment'
        if len(reasons) == 0:
            reasons['other'] = 'Consult with your manager for feedback'

        text = 'Sorry, you are not eligible for promotion'
        is_promoted = 0

        training_recommendations = {reason: recommendation for (reason, recommendation) in reasons.items()}
...
if __name__ == '__main__':
    app_dynamic.run(debug=True,port=5000)
